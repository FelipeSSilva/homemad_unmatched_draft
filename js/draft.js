...
function draft_status() {
    ...
    for (let i = 0; i < draft.draft.log.length; i++) {
        ...
        show_value = log_item.value;

        if(log_item.category === 'map'){
            $('#player-' + p.id + ' .chosen-' + log_item.category).html(show_value);
        }

        if(log_item.category === 'hero'){
            const hero_names_html = p.heroes.join('<br>');

            $('#player-' + p.id + ' .chosen-' + log_item.category).html(hero_names_html);
        }
